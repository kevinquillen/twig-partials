...
Register the extension in your module services file:

```yaml
services:
  my_module.twig_partial_extension:
    class: TwigPartials\Extension\PartialExtension
    tags:
      - { name: twig.extension }
```

To enable fragment rendering, decorate the Twig loader:

```yaml
services:
  my_module.twig_partial_loader:
    class: TwigPartials\Loader\PartialLoader
    decorates: twig.loader.filesystem
    arguments: ['@my_module.twig_partial_loader.inner']
```
